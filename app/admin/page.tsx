...
async function getStats() {
  const [
    ...
    prisma.vote.aggregate({ where: { status: "paid" }, _sum: { quantity: true, amount: true } }),
    prisma.ticket.count({ where: { status: "paid" } }),
    prisma.ticket.aggregate({ where: { status: "paid" }, _sum: { amount: true } }),
    prisma.vote.groupBy({
...
export default async function AdminOverviewPage() {
  ...
  const statCards = [
    { label: "Total Transactions", value: stats.totalVoteCount.toLocaleString(), sub: "paid vote transactions" },
    { label: "Votes Placed", value: stats.totalVotesPlaced.toLocaleString(), sub: "individual votes cast" },
    { label: "Vote Revenue", value: formatNaira(stats.voteRevenueKobo), sub: "from voting" },
    { label: "Tickets Sold", value: stats.totalTicketCount.toLocaleString(), sub: `revenue: ${formatNaira(stats.ticketRevenueKobo)}` },
  ]
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="rounded-2xl border border-white/8 bg-card p-6">
        <h2 className="mb-4 text-sm font-bold text-white">Top Voted Nominees</h2>
        {stats.topVotes.length === 0 ? (
          ...
          <div className="space-y-3">
            {stats.topVotes.map((v, i) => {
              ...
              const contestant = cat?.contestants.find((c) => c.id === v.contestantId)
              return (
                <div key={`${v.categoryId}-${v.contestantId}`} className="flex items-center gap-4">
                  <span className="w-6 text-xs font-bold text-white/20">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <p className="truncate text-sm font-semibold text-white">
                      {contestant?.name ?? v.contestantId}
                    </p>
                    <p className="text-xs text-white/30">{cat?.name ?? v.categoryId}</p>
                  </div>
