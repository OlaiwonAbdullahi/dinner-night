...
export default async function TicketReceiptPage({
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&color=D4AF37&bgcolor=0d0d0d&data=${encodeURIComponent(reference)}`
    ...
    <>
      ...
      <style>{`
        @media print {
          ...
          .ticket-card { box-shadow: none !important; border: 2px solid #D4AF37 !important; }
        }
      `}</style>
      ...
      <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4 py-12">
        ...
        <div className="no-print mb-8 flex w-full max-w-lg items-center justify-between">
          <Link href="/" className="text-xs font-extrabold tracking-widest text-white">
        ...
        {/* Ticket card */}
        <div
          className="ticket-card w-full max-w-lg overflow-hidden rounded-3xl border border-primary/40 bg-[#0d0d0d] shadow-[0_0_60px_oklch(0.745_0.14_86/0.15)]"
        >
          {/* Header band */}
          ...
          <div className="px-8 py-6">
            <div className="grid grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                ...
                <Detail label="Ticket Type" value={tier?.name ?? ticket.tierId} highlight />
                <Detail label="Date" value="June 18, 2026" />
                <Detail label="Venue" value="Antimaggies event center, Yoaco Ogbomosho" />
                <Detail label="Amount Paid" value={`₦${(ticket.amount / 100).toLocaleString()}`} />
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-3">
                ...
                />
                <p className="text-[10px] text-white/30 text-center">Scan at entrance</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between px-8 py-4">
            <div>
              <p className="text-[10px] text-white/25 uppercase tracking-widest">Ticket No.</p>
              <p className="font-mono text-sm font-bold text-primary">{ticketNumber}</p>
            </div>
            <div className="text-right">
              <p className="text-[10px] text-white/25 uppercase tracking-widest">Ref</p>
...
function Detail({
  ...
  return (
    <div>
      <p className="text-[10px] font-semibold tracking-widest text-white/25 uppercase">{label}</p>
      <p className={`mt-0.5 text-sm font-bold ${highlight ? "text-primary" : "text-white"}`}>
        {value}
      </p>
    </div>
  )
